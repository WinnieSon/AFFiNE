...
async function deleteAllDemoWorkspaces() {
  ...
  try {
    ...
    for (const db of databases) {
      if (
        ...
        try {
          ...
          console.error(`Failed to delete database ${db.name}:`, _);
        }
      }
    }
  } catch (err) {
    ...
    const knownPatterns = ['affine-local', 'affine-meta', 'affine-migration'];
  ...
  if (
    confirm('Demo workspaces deleted. Would you like to reload the page now?')
  ) {
