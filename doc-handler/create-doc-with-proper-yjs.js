...
// Create a proper Yjs document for AFFiNE
function createAffineDoc(title = 'New Document') {
  const doc = new Y.Doc();
  ...
  pageBlock.set('prop:title', new Y.Text(title));

  blocks.set(pageId, pageBlock);

  // Create a surface block (required for AFFiNE)
  const surfaceId = 'surface:home';
  ...
  // Add surface to page children
  const pageChildren = pageBlock.get('sys:children');
  pageChildren.push([surfaceId]);

  // Create a note block
  const noteId = 'note:home';
  const noteBlock = new Y.Map();
  noteBlock.set('sys:id', noteId);
  noteBlock.set('sys:flavour', 'affine:note');
  noteBlock.set('sys:children', new Y.Array());
  noteBlock.set('prop:xywh', '[0,0,800,95]');
  noteBlock.set('prop:background', '--affine-background-secondary-color');
  ...
  const paragraphId = 'paragraph:home';
  ...
  paragraphBlock.set('prop:text', new Y.Text('Hello from REST API!'));

  blocks.set(paragraphId, paragraphBlock);

  // Add paragraph to note children
...
async function createDocument(workspaceId, cookie) {
  ...
  // Create a proper Yjs document
  const doc = createAffineDoc('Document created via REST API with Yjs');
  const update = Y.encodeStateAsUpdate(doc);

  console.log('📝 Yjs document created, size:', update.length, 'bytes');
...
async function main() {
  try {
    console.log('🚀 AFFiNE REST API Document Creation Test (with proper Yjs)');
    console.log('=========================================================\n');
